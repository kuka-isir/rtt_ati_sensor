...
bool rtt_ati::FTSensor::configureHook(){
    ...
    if(rosparam){
        configured = rosparam->getParam("/" + this->getName() + "/ip","ip");
        ...
        configured = rosparam->getParam("/" + this->getName() + "/calibration_index","calibration_index");
        ...
        configured = rosparam->getParam("/" + this->getName() + "/frame","frame");
    }else if(ip_.empty()){
        ip_ = ati::default_ip;
